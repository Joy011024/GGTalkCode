...
namespace GGTalk.UnitViews
{
    public partial class UserListBox : UserControl
    {
        ...
        {
            [Description("邀请加入群聊")]
            InviteMemberAddTeam=1,
            [Description("邀请成员加入讨论组")]
            InviteMemberAddGroup=2
        }
        /// <summary>
        /// 邀请人员加入群组
        /// </summary>
        /// <param name="teamGroupId"></param>
        public void ShowInviteMemberIntoTeam(int teamGroupId)
        {
            //邀请成员加入群
            //是否已经存在了菜单项
            //枚举提取：key和描述
        ...
        public void ShowInvoteMemberIntoGroup(int groupId)
        {

        }
        private void LogicAddItem(string itemName,string itemKey)
        {
            foreach (ToolStripMenuItem item in cmsGoupMenuItem.Items)
            {
                object tag = item.Tag;
            ...
            menu.Click += new EventHandler(ClickEvent);
            cmsGoupMenuItem.Items.AddRange(new System.Windows.Forms.ToolStripItem[] { menu });
        }
