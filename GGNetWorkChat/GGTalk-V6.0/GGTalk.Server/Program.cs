    ...
    public class AppConfig
    {
        ...
        public static string AppCompany
        {
            get
            {
                ...
                {
                    companyName = ConfigurationManager.AppSettings["AppCompanyName"];
                }
                return companyName;
            }
        }
        ...
        public static string AppCompanyUrl
        {
            get {
                ...
                {
                    companyUrl = ConfigurationManager.AppSettings["AppCompanyUrl"];
                }
                return companyUrl;
            }
        }
        static string appStartYear;
        public static string AppStartYear
        {
            get
            {
                if (string.IsNullOrEmpty(appStartYear))
                {
                    appStartYear = ConfigurationManager.AppSettings["AppStartYear"];
                }
                return appStartYear;
            }
        }
        static string appMaxLinkNum;
        public static int AppMaxLinkNum
        {
            get
            {
                if (string.IsNullOrEmpty(appMaxLinkNum))
                {
                    appMaxLinkNum = ConfigurationManager.AppSettings["AppMaxOnlinLinkNum"];
                }
                int max = 0;
                if (int.TryParse(appMaxLinkNum, out max))
                { }
                if (max < 1)
                {
                    max = 1024;
